...
Options SanitizeOptions(const std::string& dbpath, const std::string& dbname,
	...
	if (result.info_log == NULL) {
		...
		src.env->CreateDir(dbpath);  // In case it does not exist
...
DBImpl::DBImpl(const Options& raw_options, const std::string& dbpath,
		...
		env_(raw_options.env), internal_comparator_(raw_options.comparator), internal_filter_policy_(
				...
				SanitizeOptions(dbpath, dbname, &internal_comparator_,
						&internal_filter_policy_, raw_options)), owns_info_log_(
				options_.info_log != raw_options.info_log), owns_cache_(
				options_.block_cache != raw_options.block_cache), dbpath_(
				dbpath), dbname_(dbname), db_lock_(NULL), shutting_down_(NULL), bg_cv_(
				&mutex_), mem_(NULL), imm_(NULL), logfile_(NULL), logfile_number_(
				0), log_(NULL), seed_(0), tmp_batch_(new WriteBatch), bg_compaction_scheduled_(
	...
	table_cache_ = new TableCache(dbpath_, dbname_, &options_,
			table_cache_size);

	versions_ = new VersionSet(dbpath_, dbname_, &options_, table_cache_,
...
DBImpl::~DBImpl() {
	...
	LOGE(db_lock_ != NULL ? "db_lock_ != NULL" : "db_lock_ == NULL");
...
void DBImpl::DeleteObsoleteFiles() {
	...
	std::vector < std::string > filenames;
	env_->GetChildren(dbpath_, dbname_, &filenames); // Ignoring errors on purpose
	uint64_t number;
	FileType type;
	for (size_t i = 0; i < filenames.size(); i++) {
		if (ParseFileName(dbpath_, filenames[i], &number, &type)) {
...
//				keep = (number >= versions_->ManifestFileNumber());
			...
			if (!keep) {
				LOGW("have file need delete");
				LOGW(filenames[i].data());
				if (type == kTableFile) {
...
Status DBImpl::Recover(VersionEdit* edit, bool *save_manifest) {
	...
	env_->CreateDir(dbpath_);
	assert(db_lock_ == NULL);
	Status s = env_->LockFile(LockFileName(dbpath_, dbname_), &db_lock_);
	if (!s.ok()) {
		return s;
	}
	...
	for (size_t i = 0; i < logs.size(); i++) {
		...
		versions_->MarkFileNumberUsed(1);
	}
...
Status DBImpl::RecoverLogFile(std::string log_name, bool last_log,
	...
	// Open the log file
	std::string fname = dbpath_ + "/" + log_name;
	LOGE(fname.data());
	SequentialFile* file;
	...
	Log(options_.info_log, "Recovering log #%s", log_name.data());
...
Status DBImpl::WriteLevel0Table(MemTable* mem, VersionEdit* edit,
	...
	{
		mutex_.Unlock();
		s = BuildTable(dbpath_, dbname_, env_, options_, table_cache_, iter,
				&meta);
		mutex_.Lock();
	}
...
Status DBImpl::Put(const WriteOptions& o, const Slice& key, const Slice& val) {
	if (options_.read_only)
		return Status::NotSupported("read only mode, can not put");
...
Status DBImpl::Delete(const WriteOptions& options, const Slice& key) {
	if (options_.read_only)
		return Status::NotSupported("read only mode, can not delete");
...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* my_batch) {
	...
	if (options_.read_only)
		return Status::NotSupported("read only mode, can not wirte");
...
Status DB::Open(const Options& options, const std::string& dbpath,
	...
	*dbptr = NULL;
	...
	if (s.ok()) {
		impl->DeleteObsoleteFiles();	//对文件编号进行了检查，不对的会删除
...
Status DestroyDB(const std::string& dbpath, const std::string& dbname,
		const Options& options) {
	Env* env = options.env;
	std::vector < std::string > filenames;
	// Ignore error in case directory does not exist
	env->GetChildren(dbpath, dbname, &filenames);
	if (filenames.empty()) {
		return Status::OK();
	}

	FileLock* lock;
	const std::string lockname = LockFileName(dbpath, dbname);
	Status result = env->LockFile(lockname, &lock);
	if (result.ok()) {
		uint64_t number;
		FileType type;
		for (size_t i = 0; i < filenames.size(); i++) {
			if (ParseFileName(dbpath, filenames[i], &number, &type)
