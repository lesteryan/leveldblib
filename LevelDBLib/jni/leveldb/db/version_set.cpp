...
VersionSet::VersionSet(const std::string& dbpath, const std::string& dbname,
		const Options* options, TableCache* table_cache,
		const InternalKeyComparator* cmp) :
		env_(options->env), dbpath_(dbpath), dbname_(dbname), options_(options), table_cache_(
				table_cache), icmp_(*cmp), next_file_number_(2), manifest_file_number_(
				0),  // Filled by Recover()
		last_sequence_(0), log_number_(0), prev_log_number_(0), descriptor_file_(
				NULL), descriptor_log_(NULL), dummy_versions_(this), current_(
				NULL) {
...
//  current.resize(current.size() - 1);
	...
	std::string dscname = DescriptorFileName(dbpath_, dbname_, 1);
	SequentialFile* file;
	...
	if (s.ok()) {
		...
		if (ReuseManifest(dbpath_, dscname, std::string(""))) {
...
bool VersionSet::ReuseManifest(const std::string& dscpath,
		const std::string& dscname, const std::string& dscbase) {
...
//  }
	...
	if (!env_->GetFileSize(dbpath_ + dscname, &manifest_size).ok() ||
	// Make new compacted MANIFEST if old one is too big
			manifest_size >= kTargetFileSize) {
		return false;
	}
...
Compaction::Compaction(int level) :
		level_(level), max_output_file_size_(MaxFileSizeForLevel(level)), input_version_(
				NULL), grandparent_index_(0), seen_key_(false), overlapped_bytes_(
				0) {
